<div class="bg-gradient-to-b from-transparent from-20% via-blue-400 to-blue-700 dark:via-40% dark:via-blue-400 dark:to-blue-700 flex items-center justify-center h-screen"
    style="height: 200px;">
    <div class="max-w-6xl mx-auto mt-14 flex items-center">
        <div class="mr-10">
            <img src="http://127.0.0.1:8000/storage/images/logo.png" class="h-20" alt="Logo">
        </div>
        <div>
            <h2 class="text-lg font-bold text-white dark:text-gray-100">Desarrollado Por</h2>
            <p class="text-sm text-white dark:text-gray-200">Jeeimy Pantoja | Marlon Jaramillo | Sebastian Pai |
                Sebastian Figueroa</p>
            <p class="text-sm text-white dark:text-gray-200">jeeimyda.pantoja@umariana.edu.co |
                marlonfe.jaramillo@umariana.edu.co | jpai@umariana.edu.co | juafigueroa@umariana.edu.co</p>

        </div>
    </div>
</div>
